...
const Search = () => {
  ...
  useEffect(() => {
    axios.get(`https://api.themoviedb.org/3/search/${searchType}?query=${params.get('query')}&api_key=${key}${currentStartValue ? `&page=${currentPage}`:``}`)
    .then((response)=>{
  ...
  const handleNextPage = () =>{
    ...
    router.push( `/search?query=${params.get('query')}&pageNo=${++currentPage}`)
    }
  }   
  ...
  return (
    <div>
      <div className=' pt-20 lg:pt-0 flex flex-col gap-4 w-screen'>
      <div className='pl-2 flex flex-col gap-4'>
        <h1 className='text-3xl text-gray-300  font-bold mt-6'>Tmdb Search</h1>
          <div className='flex '>
        <SearchInput />
        ...
        <div className='grid grid-cols-4 w-[79%]'>
          {(data?.map((elem)  => (
      ...
      <div className='flex items-center justify-between mt-6 w-[79%]' >
          <div className='flex flex-col' >
            ...
            <div onClick={handleNextPage} className={`${currentPage >= (completeData?.total_pages ??  0) ? 'hidden': '' }`}>
