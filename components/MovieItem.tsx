...
const MovieItem = ({movie,type}:{movie:any,type:string}) => {  
  ...
  return (
    ...
    <div className={`relative
     ...
     inline-block rounded-lg overflow-hidden cursor-pointer m-2 transition-all duration-200   ${pathname=='/' && 'w-[180px] lg:w-[200px] 2xl:w-[250px] 2xl:hover:w-[270px]  lg:h-[300px] xl:h-[400px]'}`}>
     <img
     ...
     className='absolute h-full top-0 left-0 w-full transition-all bg-black/80 backdrop-blur-md opacity-0 hover:opacity-100 p-3  xl:pl-3 group'>
        ...
        ${pathname=='/nextpage' ? ' group-hover:leading-[28px] text-[20px] lg:text-[28px]' :' group-hover:lg:leading-[36px] lg:text-[36px] leading-[20px] text-[20px]'} 
         `}
        >{title ? title : name}</p>
        <p 
